...
export default function RepDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: logsData } = await supabase.from('daily_logs').select('*').eq('rep_id', id).order('log_date', { ascending: false }).limit(60)
      const { data: t } = await supabase.from('kpi_targets').select('*')
      setRep(repData); setLogs(logsData || []); setTargets(t || []); setLoading(false)
    }
    load()
  }, [id])
  ...
  const weeklyPct = calcCompletionPct(sumLogs(filterLogsByPeriod(logs, 'weekly')), getTarget('weekly'))
  return (
    <div className="p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900">{rep.full_name}</h1><p className="text-gray-400 text-sm">{rep.email}</p></div>
        <div className="ml-auto text-right"><p className={clsx('text-3xl font-bold', getStatusColor(weeklyPct))}>{weeklyPct}%</p><p className={clsx('text-sm font-medium', getStatusColor(weeklyPct))}>{getStatusLabel(weeklyPct)}</p></div>
      </div>
      {periods.map(period => {
        const totals = sumLogs(filterLogsByPeriod(logs, period))
        ...
        return (
          <div key={period} className="card p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2"><div className="h-1.5 w-24 bg-gray-100 rounded-full overflow-hidden"><div className={clsx('h-full rounded-full', getStatusBg(pct))} style={{ width: `${pct}%` }} /></div><span className={clsx('text-sm font-semibold', getStatusColor(pct))}>{pct}%</span></div>
            </div>
