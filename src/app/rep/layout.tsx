...
export default function RepLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data.role === 'manager') { window.location.href = '/dashboard'; return }
  ...
  return <div className="flex min-h-screen"><Sidebar profile={profile} /><main className="flex-1 overflow-y-auto">{children}</main></div>
}
